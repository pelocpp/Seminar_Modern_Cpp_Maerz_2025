...
namespace Auto_Examples {
    ...
    static void test_00() {
        ...
        auto n = 123.456F;  // Type Deduction
                            // Typableitung
    }
    ...
    //auto tueWas(bool flag, float n, double d) -> double
    //{
    //    if (flag) {
    //        return d;
    //    }
    ...
    auto tueWas2(bool flag, T n, U d) -> decltype (n + d)
    {
        if (flag) {
            return d;
        }
    ...
    decltype (  std::declval<T>() + std::declval<U>() )
        tueWas(bool flag, T n, U d)
    {
        decltype (std::declval<T>() + std::declval<U>()) tmp;
    ...
    void test_tue_was()
    {
        auto result1 = tueWas (true, 123.0f, 456l);  // Type Deduction

        auto result2 = tueWas<>(false, 123, 456);  // Type Deduction
    }
    ...
    static const std::string& getMessage()  // 90% Frage: Wert
    {
        return message;
    }
